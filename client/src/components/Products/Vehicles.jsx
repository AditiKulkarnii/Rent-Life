...
function Vehicles() {
    const { vehicleProducts } = useContext(ProductsContext);
  ...
  useEffect(() => {
    if (searchQuery) {
        ...
        const filtered = vehicleProducts.filter(product =>
            product.ProductName.toLowerCase().includes(searchQuery.toLowerCase())
        );
        applyFilters(filtered);
    } else {
        // If no search query, display all products
        applyFilters(vehicleProducts);
    }
}, [vehicleProducts, searchQuery, filters1]);
...
if (!vehicleProducts) {
    ...
    useEffect(() => {
        if (searchQuery) {
            ...
            // If no search query, display all products
            setFilteredProducts(vehicleProducts);
        }
    }, [vehicleProducts, searchQuery]);
