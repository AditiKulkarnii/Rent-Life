...
function Music() {
    const { musicalInstrumentProducts } = useContext(ProductsContext);
  ...
  useEffect(() => {
    if (searchQuery) {
        ...
        const filtered = musicalInstrumentProducts.filter(product =>
            product.ProductName.toLowerCase().includes(searchQuery.toLowerCase())
        );
        applyFilters(filtered);
    } else {
        // If no search query, display all products
        applyFilters(musicalInstrumentProducts);
    }
}, [musicalInstrumentProducts, searchQuery, filters1]);
...
if (!musicalInstrumentProducts) {
    ...
    useEffect(() => {
        if (searchQuery) {
            ...
            // If no search query, display all products
            setFilteredProducts(musicalInstrumentProducts);
        }
    }, [musicalInstrumentProducts, searchQuery]);
