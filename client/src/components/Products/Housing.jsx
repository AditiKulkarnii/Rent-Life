...
function Housing() {
  ...
  useEffect(() => {
    const unsubscribe = db.collection('House').onSnapshot(snapshot => {
  ...
  useEffect(() => {
    if (searchQuery) {
        ...
        const filtered = housingProducts.filter(product =>
            product.ProductName.toLowerCase().includes(searchQuery.toLowerCase())
        );
        setFilteredProducts(filtered);
    } else {
        // If no search query, display all products
        setFilteredProducts(housingProducts);
    }
}, [housingProducts, searchQuery]);
...
useEffect(() => {
    if (searchQuery) {
        ...
        // If no search query, display all products
        applyFilters(housingProducts);
    }
}, [housingProducts, searchQuery, filters1]);
        ...
        {currentItems.map(product => (
          <div key={product.ProductID} className="m-4">
            <div className="w-50 p-4 bg-white rounded-xl shadow-xl hover:shadow-gray-500 hover:shadow-2xl transform duration-500">
              ...
              {/* End wishlist */}
              <Link to={`/housing/${product.ProductID}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                ...
                <div className="mt-4">
                  ...
                  <p className="text-gray-700 mt-2">{product.ProductDes.split(' ').slice(0, 10).join(' ')}... <Link to={`/housing/${product.ProductID}`} className="text-blue-500">View More</Link></p>

                  {/* Icons */}
                  <div className="flex items-center mt-2">
                    <FontAwesomeIcon icon={faBed} className="text-gray-500 mr-2" />
                    <span className="text-gray-700">{product.Bedrooms} Beds</span>

                    <FontAwesomeIcon icon={faBath} className="text-gray-500 mx-2" />
                    <span className="text-gray-700">{product.Bathrooms} Baths</span>

                    <FontAwesomeIcon icon={faCouch} className="text-gray-500 mx-2" />
                    <span className="text-gray-700">{product.Furnished} Furnished</span>
                  </div>
